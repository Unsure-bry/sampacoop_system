...
export default function SecretaryReportsPage() {
  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      const archivedMembers = members.filter((m: any) => m.archived === true);
      ...
      const savingsPromises = members.map(async (member: any) => {
        try {
          ...
          if (savingsResult.success && savingsResult.data) {
            let filteredTransactions = [...savingsResult.data];
            if (dateRange.start || dateRange.end) {
      ...
      // Process loans summary
      const activeLoans = loans.filter((loan: any) => loan.status === 'active' || loan.status === 'approved');
      const completedLoans = loans.filter((loan: any) => loan.status === 'completed' || loan.status === 'paid');
      ...
      loans.forEach((loan: any) => {
        if (loan.paymentSchedule && Array.isArray(loan.paymentSchedule)) {
          loan.paymentSchedule.forEach((payment: any) => {
            if (payment.status === 'paid' && payment.paidAmount) {
              totalPaymentsReceived += payment.paidAmount;
              if (payment.principal) {
                totalPrincipalCollected += payment.principal;
              }
              if (payment.interest) {
                totalInterestCollected += payment.interest;
              }
            }
          });
        }
            ...
            ${reportData ? `
              ...
              <div class="section">
                ...
                <div class="stats-grid">
                  ...
                  <div class="stat-card">
                    <div class="stat-value">${reportData.membersSummary.archivedMembers}</div>
              ...
              <div class="section">
                ...
                <div class="stats-grid">
                  ...
                  <div class="stat-card">
                    <div class="stat-value">${reportData.loansSummary.completedLoans}</div>
              ...
              <div class="section">
                ...
                <div class="stats-grid">
                  <div class="stat-card">
                    <div class="stat-value">₱${reportData.paymentSummary.totalPaymentsReceived.toLocaleString()}</div>
                    <div class="stat-label">Total Payments Received</div>
                  </div>
                  <div class="stat-card">
                    <div class="stat-value">₱${reportData.paymentSummary.totalPrincipalCollected.toLocaleString()}</div>
                    <div class="stat-label">Total Principal Collected</div>
                  </div>
                  <div class="stat-card">
                    <div class="stat-value">₱${reportData.paymentSummary.totalInterestCollected.toLocaleString()}</div>
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-gray-600">{reportData.membersSummary.archivedMembers}</div>
                    <div className="text-sm text-gray-800">Archived Members</div>
                  </div>
                </div>
            ...
            {activeTab === 'loans' && (
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">{reportData.loansSummary.completedLoans}</div>
                    <div className="text-sm text-green-800">Completed Loans</div>
                  </div>
                  <div className="bg-orange-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-orange-600">₱{reportData.loansSummary.totalLoanAmount.toLocaleString()}</div>
                    <div className="text-sm text-orange-800">Total Loan Amount</div>
                  </div>
                </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-teal-50 p-4 rounded-lg">
                      <div className="text-2xl font-bold text-teal-600">₱{reportData.paymentSummary.totalPaymentsReceived.toLocaleString()}</div>
                      <div className="text-sm text-teal-800">Total Payments Received</div>
                    </div>
                    <div className="bg-indigo-50 p-4 rounded-lg">
                      <div className="text-2xl font-bold text-indigo-600">₱{reportData.paymentSummary.totalPrincipalCollected.toLocaleString()}</div>
                      <div className="text-sm text-indigo-800">Total Principal Collected</div>
                    </div>
                    <div className="bg-pink-50 p-4 rounded-lg">
                      <div className="text-2xl font-bold text-pink-600">₱{reportData.paymentSummary.totalInterestCollected.toLocaleString()}</div>
                      <div className="text-sm text-pink-800">Total Interest Collected</div>
                    </div>
                  </div>
                </div>

...
export default function SecretaryLayout({
  ...
  useEffect(() => {
    if (!loading && user && user.role?.toLowerCase() !== 'secretary') {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className={`bg-white shadow-md ${sidebarCollapsed ? 'w-64' : 'w-20'} transition-all duration-300 ease-in-out`}>
        <SecretarySidebar sidebarCollapsed={sidebarCollapsed} />
      </div>
        ...
        <header className="bg-white shadow">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-semibold text-gray-800">Secretary Dashboard</h1>
            </div>
            <div className="flex items-center space-x-4">
              <span className="text-gray-700">Welcome, {user?.email || 'Secretary'}</span>
              <button

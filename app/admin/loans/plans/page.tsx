  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-black">Loan Plans</h1>
          <p className="text-black">Manage available loan plans</p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-black">Available Loan Plans</h2>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="relative">
            <select
              ...
              className={`w-full p-3 pr-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 appearance-none bg-white cursor-pointer ${selectedPlanId ? 'text-black' : 'text-gray-500'}`}
            >
              <option value="" disabled hidden>-- Select Loan Plans --</option>
              {loanPlans.map((plan) => (
          ...
          <div className="border-t border-gray-200">
            <div className="p-6">
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-black">{selectedPlan.name}</h3>
                  <p className="text-black mt-1">{selectedPlan.description}</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-xs text-black uppercase tracking-wide mb-1">Maximum Amount</p>
                  <p className="text-lg font-semibold text-black">{formatCurrency(selectedPlan.maxAmount)}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-xs text-black uppercase tracking-wide mb-1">Interest Rate</p>
                  <p className="text-lg font-semibold text-black">{selectedPlan.interestRate}%</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-xs text-black uppercase tracking-wide mb-1">Term Options</p>
                  <p className="text-lg font-semibold text-black">{selectedPlan.termOptions.join(', ')} <span className="text-sm font-normal text-black">months</span></p>
                </div>
              </div>
        ...
        {!selectedPlan && (
          <div className="border-t border-gray-200 p-8 text-center">
            ...
            <p className="text-black">Select a loan plan from the dropdown to view details</p>
          </div>
        )}
      </div>

...
export default function MemberEditModal({ 
  ...
  useEffect(() => {
    if (member && isOpen) {
      reset({
        ...
        driverLicenseNumber: member.driverInfo?.licenseNumber || '',
        driverTinId: member.driverInfo?.tinId || '',
        driverHouseNumber: member.driverInfo?.houseNumber || '',
        driverBlockNumber: member.driverInfo?.blockNumber || '',
        driverLotNumber: member.driverInfo?.lotNumber || '',
        driverStreet: member.driverInfo?.street || '',
        driverBarangay: member.driverInfo?.barangay || '',
        driverCity: member.driverInfo?.city || '',
        operatorLicenseNumber: member.operatorInfo?.licenseNumber || '',
        operatorTinId: member.operatorInfo?.tinId || '',
        operatorHouseNumber: member.operatorInfo?.houseNumber || '',
        operatorBlockNumber: member.operatorInfo?.blockNumber || '',
        operatorLotNumber: member.operatorInfo?.lotNumber || '',
        operatorStreet: member.operatorInfo?.street || '',
        operatorBarangay: member.operatorInfo?.barangay || '',
        operatorCity: member.operatorInfo?.city || '',
        numberOfJeepneys: member.operatorInfo?.numberOfJeepneys || 0,
        plateNumbers: member.operatorInfo?.plateNumbers || []
      });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const memberResult = await firestore.updateDocument(
        'members',
        member.id,
        memberData
      );
      ...
      if (memberResult.success) {
        ...
        setRole(null);
        onClose();
        onMemberUpdated(); // This will trigger the refresh in the Membership page
      } else {
